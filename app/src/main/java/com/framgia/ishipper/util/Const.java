package com.framgia.ishipper.util;
/**
 * Created by vuduychuong1994 on 8/3/16.
 */
public class Const {

    public static final String DEBUG_TAG = "iShipper Debug";

    public static final String SERVER_URL = "http://192.168.1.118:3001/";
    public static final String SIGNUP_PATH = "api/sign_up/";
    public static final String GET_NEARBY_PATH = "api/users";



    public static final String SIGNOUT_PATH = "api/sign_out/";
    public static final String USER_PHONE_NUMBER = "user[phone_number]";
    public static final String USER_PASSWORD = "user[password]";
    public static final String USER_PASSWORD_CONFIRMATION = "user[password_confirmation]";
    public static final String USER_TOKEN = "authentication_token";
    public static final String ROLE = "user[role]";
    public static final String NAME = "user[name]";
    public static final String EMAIL = "user[email]";

    public static String VIETNAM_PREFIX = "+84";

}
